...
export default function CallForSponsors() {
  ...
  return (
    <div>
        <CheetoLeft className={classes.cheetohLeftMirrored} style={{'margin-top':'-1em'}} alt="cheetohLeft" />
        <TextBox style={{'margin-top':'135px' , 'margin-right':'calc(37vw - 138px)' , 'padding-right':'0'}}>
            <BoldedHeader headerText={headerText} style={{'font-size':'calc(1.3vw + 1em)'}}/>
            <Content content={content} style={{'text-align':'left' , 'margin-top':'-5px'}}/>
            <CheetoRight className={classes.cheetohRightMirrored} style={{'margin-top':'calc(1vh + 1px)' , 'margin-right':'auto'}} alt="cheetohRight"/>
        </TextBox>
        <SkylineHeaderSVG />
    </div>
  )
